...
use geo::Coord;
use geo_types::{MultiPolygon, Polygon, LineString};
...
impl ToMesh3D for MultiPolygon<f64> {
    fn to_mesh3d_on_sphere(
        ...
        let mut vertices: Vec<[OrderedFloat<f64>; 3]> = Vec::new();
        let mut normals: Vec<[f64; 3]> = Vec::new();
        let mut indices: Vec<[u32; 3]> = Vec::new();
        let mut colors: Vec<u32> = Vec::new();
        let mut vertex_map: HashMap<[OrderedFloat<f64>; 3], u32> = HashMap::new();
        let mut vertex_index: u32 = 0;
        let precision = 1e-9; // Adjust precision as needed
        ...
        for polygon in &self.0 {
            ...
            let triangles = match subdivided_polygon.constrained_triangulation(config) {
                ...
                Err(_) => continue, // Skip polygons that cannot be triangulated
            };
...
fn main() {
    ...
    let subdivision_depth = 2; // Adjust depth as needed (higher values increase detail)

    let rec = rerun::RecordingStreamBuilder::new("earth_example")
        .connect()
        .expect("Error connecting to Rerun");

    let mut reader = shapefile::Reader::from_path("50m_land/ne_50m_land.shp")
    ...
    for (i, shape_record) in reader.iter_shapes_and_records().enumerate() {
        ...
        let mesh = multi_polygon.to_mesh3d_on_sphere(sphere_radius, 0x00FF00FF, max_subdivision_length, subdivision_depth);
        rec.log_static(format!("land/{i}"), &mesh)
            .expect("Error logging mesh");
    }

    let mut reader = shapefile::Reader::from_path("50m_ocean/ne_50m_ocean.shp")
        .expect("Error reading shapefile");
    ...
    for (i, shape_record) in reader.iter_shapes_and_records().enumerate() {
        ...
        let mesh = multi_polygon.to_mesh3d_on_sphere(sphere_radius, 0x0000FFFF, max_subdivision_length, subdivision_depth);
        rec.log_static(format!("ocean/{i}"), &mesh)
            .expect("Error logging mesh");
    }
}


fn lat_lon_to_xyz(lat_deg: f32, lon_deg: f32, radius: f32) -> [f32; 3] {
